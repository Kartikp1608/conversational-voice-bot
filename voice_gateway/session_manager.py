...
class SessionManager:
    ...
    def create_session(self, session_id: str, call_id: str, prompt_id: str, direction: str = "inbound") -> VoiceSession:
        session = VoiceSession(session_id, call_id, prompt_id, direction)
        self._sessions[session_id] = session
        ACTIVE_CALLS.set(len(self._sessions))
        logger.info(f"Created active voice session {session_id}", session_id=session_id, call_id=call_id)
        return session
