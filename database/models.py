...
class CallSession(Base):
    ...
    status: Mapped[str] = mapped_column(String(32), default="initiated")  # initiated, connected, ended, failed
    ...
    transcripts: Mapped[list["TranscriptSegment"]] = relationship("TranscriptSegment", back_populates="session", cascade="all, delete-orphan")
    analytics: Mapped[Optional["CallAnalytics"]] = relationship("CallAnalytics", back_populates="session", uselist=False, cascade="all, delete-orphan")
    tool_logs: Mapped[list["ToolExecutionLog"]] = relationship("ToolExecutionLog", back_populates="session", cascade="all, delete-orphan")
...
class TranscriptSegment(Base):
    ...
    call_id: Mapped[str] = mapped_column(String(64), ForeignKey("call_sessions.call_id"), nullable=False)
...
class CallAnalytics(Base):
    ...
    call_id: Mapped[str] = mapped_column(String(64), ForeignKey("call_sessions.call_id"), primary_key=True)
